...
- **Inf handling in `_get_frob_mask` runs on the subsample, not the full volume.** Infs would break `triangle_threshold` / `otsu_threshold` (both call `xp.histogram(range=(min, max))`). Two iterations: first the full-volume copy was dropped (excluded infs from the threshold input via boolean indexing); then ([[decisions/0012-filter-frob-mask-subsample-side-inf-filter|ADR 0012]]) the inf-filter itself moved from the full volume to the subsample — the threshold is computed from a `chunking.subsample_for_thresholds` result anyway, so `xp.isfinite(positive_subsample)` is the smallest array that has to be inf-clean. Common case (real microscopy data — finite Hessian via `compute_hessian`'s `max_abs > 0` guard): zero full-volume `xp.isinf` scans per sigma. Pathological all-inf-subsample case falls back to a full-volume `xp.isinf().all()` to preserve the all-inf → all-False contract. Bit-identical for finite-only data; approx-equivalent (sampling-noise on the threshold) for inf-containing data.
- **Per-sigma dispatch fused into single `xp.sum(h_mask)`.** Pre-rewrite `_compute_vesselness` paid `xp.any(h_mask)` (skip-check) + `bool(h_mask.all())` (dense vs sparse dispatch in `_compute_vesselness_chunkwise`) — two full-volume reductions. Now `_compute_vesselness` computes `true_count = int(xp.sum(h_mask))` once, derives `is_empty` (== 0) and `is_dense` (== total), and threads `is_dense` through to `_compute_vesselness_chunkwise` via an `is_dense: bool | None = None` kwarg. Direct unit-test callers (in `tests/test_filtering.py`) pass no kwarg and fall back to the internal `bool(h_mask.all())` reduction — backward-compatible. Saves one full-volume reduction per sigma.
