...
- **Per-stage `*Config` dataclass extraction**, deferred until **all** stages have tests + canonical backend in place — then batched as a single cross-stage pass for consistency review (so individual stages like [[labelling]] that become eligible earlier still wait their turn). Each stage gets a frozen `*Config` dataclass colocated above its class, mirroring the [[filtering|`FrangiConfig`]] pattern from PRD #66. Touches the stage file + `nellie/run.py` + `nellie_napari/nellie_processor.py` + tests + wiki. [[mocap-marking|`Markers`]]' overlap was resolved in Slice 2 of PRD #84 (`prefer_gpu` dropped); [[feature-extraction|`Hierarchy`]] still has overlapping `use_gpu: bool` + `device: str | None` — resolve that before the batched config PRD.
