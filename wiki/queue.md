...
- **Per-stage backend code is duplicated across the 5 untested stages.** [[filtering]] migrated to canonical helpers in [[gpu-runtime|`adaptive_run`]] in PRD #59 (Pass 8 Separate, scope B). [[labelling]], [[networking]], [[mocap-marking]], [[hu-tracking]], [[voxel-reassignment]], plus [[feature-extraction|`hierarchical`]] still hold private duplicates of `_resolve_backend`, `_try_import_cupy`, `_is_oom_error`, `_free_gpu_memory`. Migrate each when it gains its own characterization tests; one stage per PR, each PR adds tests then migrates. Same pattern as the test bootstrap from PRD #51.
