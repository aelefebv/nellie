...
A thin sequential driver in `nellie/run.py` that wires the seven pipeline stages together against one shared `ImInfo` handle. Two callers invoke nellie this way: the CLI (`nellie/cli.py`) — which actually goes through `run.py` — and the [[processor|napari processor]], which **re-implements** the same sequence stage-by-stage rather than calling `run()`.
...
- **`run copy.py` is a stale developer scratch fork.** Not imported anywhere. Its `run()` has an older signature; the file is mostly Jupyter-style cells against a hard-coded Windows path. Safe to delete — see [[queue]].
- **CLI is broken / out-of-date with `run.py`.** `cli.py` calls `run(tif_file, ..., ch=, num_t=, output_dirpath=)` but `run.py` no longer accepts those kwargs (signature is `(file_info, …)`).
...
- **`Hierarchy` is invoked with `skip_nodes=False` hard-coded.** The napari [[settings|settings widget]] exposes it as a flag; the CLI path doesn't.
