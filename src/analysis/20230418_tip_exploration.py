...
if platform.system() == 'Windows':
    ...
    csv_path = os.path.join(top_dir, '20230418-172202-track_stats.csv')
    save_path = top_dir
else:
...
df = df.dropna()
# remove any rows with a concentration greater than 10
...
LFC = np.log2(grouped_df.loc[1] / grouped_df.loc[0])
SMDs = []
MDs = []
for col in df_volcano.columns[:-1]:
    mean1 = df_volcano[df_volcano['concentration']==0][col].mean()
    mean2 = df_volcano[df_volcano['concentration']==1][col].mean()
    sd1 = df_volcano[df_volcano['concentration']==0][col].std(ddof=1)
    sd2 = df_volcano[df_volcano['concentration']==1][col].std(ddof=1)
...
for col in df_volcano.columns[:-1]:
    pvals.append(ttest_ind(df_volcano[df_volcano['concentration']==0][col], df_volcano[df_volcano['concentration']==1][col])[1])
