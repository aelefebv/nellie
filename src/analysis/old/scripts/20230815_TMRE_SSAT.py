...
csv_path = os.path.join(top_dir, '20230803-105514-ch_0-track_stats_mutant_concentration_roi.csv')
save_path = top_dir
# else:
...
remove_cols = ['track_num', 'file_num', 'file_name', 'concentration', 'mutant', 'roi_num', 'sample_type_condition'
               ]
now = datetime.now()
...
# convert to string and concatenate the concentration and mutant columns into a new column
df['sample_type_condition'] = df['concentration'].astype(str) + '_' + df['mutant'].astype(str)
...
x = scaled_data
y = df_original['sample_type_condition']
...
# feature_cols = ['branch_intensity_q75_ch1_ratio', 'speed_med', 'branch_length_max']
...
df_to_umap['sample_type_condition'] = df_original['sample_type_condition']

df_to_umap['index'] = df.index
fig = px.scatter(df_to_umap, x='UMAP 1', y='UMAP 2', color=df_original['sample_type_condition'], hover_data=['index', 'cluster'])
                 # range_color=[0, 2])
